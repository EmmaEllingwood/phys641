...
def calculate_chisq(cosmo,dat,parameters,s):
    try:
    ...
    predicted=power[inds,0]+(s*(dat[:,0]**2))
...
chisq=calculate_chisq(cosmology,data,pars,0)
...
##changing_value=5
samples=5000
chains=np.zeros([samples,2+len(cosmology)])###Accounting for the new a_src
...
for x in range(samples):
    ...
    new_chisq=calculate_chisq(new_cosmo,data,pars,0)
    st=change_source+source*0.01*np.random.randn()
    new_chisq_src=calculate_chisq(new_cosmo,data,pars,st)
    accept=False
    if new_chisq_src<bad_chisq and new_cosmo[3]>0:
        if np.random.rand()<np.exp(-0.5*(new_chisq-chisq)):
            accept=True
    print x, ' chisq = ',new_chisq_src, ' and the step status is ',accept, 'source amplitude=',st
    print new_cosmo
    if accept:
        chisq=new_chisq
        cosmology=new_cosmo
...
np.savetxt('chains_d_trynewsource.txt',chains)
plt.figure(9)
